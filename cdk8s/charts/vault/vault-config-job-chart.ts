...
export interface VaultConfigJobChartProps extends ChartProps {
  namespace?: string;
}

export class VaultConfigJobChart extends Chart {
  constructor(scope: Construct, id: string, props: VaultConfigJobChartProps = {}) {
    super(scope, id, props);

    const namespace = props.namespace || 'vault';

    // Service Account
    const serviceAccount = new k8s.KubeServiceAccount(this, 'vault-config-sa', {
      metadata: {
        name: 'vault-config',
        namespace: namespace,
      },
    ...
    // Role
    new k8s.KubeRole(this, 'vault-config-role', {
      metadata: {
        name: 'vault-config',
        namespace: namespace,
      },
    ...
    // RoleBinding
    new k8s.KubeRoleBinding(this, 'vault-config-rolebinding', {
      metadata: {
        name: 'vault-config',
        namespace: namespace,
      },
    ...
    // Job
    new k8s.KubeJob(this, 'vault-config-job', {
      metadata: {
        name: 'vault-config',
        namespace: namespace,
        annotations: {
