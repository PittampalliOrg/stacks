...
# Discover vcluster Applications created by the ApplicationSet
vcluster_app_names=$(kubectl get application -n argocd -l cnoe.io/applicationName=vcluster-package,cnoe.io/stackName=vcluster-multi-env --no-headers -o custom-columns=":metadata.name" || true)

if [[ -z "${vcluster_app_names}" ]]; then
  echo "No vcluster Applications found yet. Ensure the ApplicationSet is synced (package: vcluster-multi-env)."
  exit 0
fi
...
echo "All vclusters are registered with Argo CD."
